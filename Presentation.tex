...
\begin{frame}{Выбор технологий}
	...
	\begin{itemize}
		...
		\item \textbf{Library Web App} (React, Redux, TypeScript) --- веб-интерфейс для читателей и библиотекарей, компонентный подход, типобезопасность
		\item \textbf{Library Mobile App} (Flutter) --- мобильное приложение, кроссплатформенность (iOS/Android), единая кодовая база
		\item \textbf{Library Desktop App} (Compose Multiplatform Desktop, Kotlin) --- нативное десктопное приложение, современный UI, производительность
	\end{itemize}
	
	\vspace{0.05cm}
	
	\textbf{Backend и инфраструктура:}
	\begin{columns}
		\column{0.5\textwidth}
		...
		\begin{itemize}
			\setlength{\itemsep}{0pt}
			\setlength{\parskip}{0pt}
			\item \textbf{Redis} --- быстрый кэш, rate-limiting

...
\begin{document}
	...
	\begin{enumerate}
		...
		\item \textbf{Пользовательские действия}
		
		Данный раздел описывает конкретные действия, которые пользователь может выполнять в системе.
		
		\textbf{3.7.1 Действия неавторизованного пользователя}
		\begin{enumerate}
			\item Пользователь может инициировать процесс регистрации в системе.
			\item Пользователь может предоставить системе регистрационные данные: ФИО, электронный адрес, пароль и подтверждение пароля.
			\item Пользователь может отправить регистрационные данные на обработку.
			\item Пользователь может инициировать процесс входа в систему.
			\item Пользователь может предоставить системе свои учётные данные (e-mail и пароль) для авторизации.
			\item Пользователь может просматривать каталог материалов без авторизации.
			\item Пользователь может выполнять поиск материалов по каталогу, указав поисковый запрос.
			\item Пользователь может применять фильтры к результатам поиска, указав параметры фильтрации (жанр, год издания, тип материала, наличие).
			\item Пользователь может сортировать результаты поиска, выбрав критерий сортировки (по алфавиту, дате издания, дате добавления).
			\item Пользователь может запрашивать детальную информацию о выбранном материале из результатов поиска.
			\item Пользователь может запросить повторную отправку письма активации учётной записи.
		\end{enumerate}
		
		\textbf{3.7.2 Действия авторизованного пользователя (читателя)}
		\begin{enumerate}
			\item Пользователь может получить доступ к своему личному кабинету после авторизации.
			\item Пользователь может просматривать список своих активных бронирований.
			\item Пользователь может инициировать бронирование выбранного материала из каталога.
			\item Пользователь может отменить созданную бронь до её подтверждения или обработки библиотекарем.
			\item Пользователь может просматривать список выданных ему материалов с информацией о дате выдачи и сроке возврата.
			\item Пользователь может просматривать историю выданных материалов.
			\item Пользователь может просматривать актуальный список своих активных штрафов с указанием суммы, причины начисления и срока оплаты.
			\item Пользователь может инициировать оплату выбранного штрафа из списка активных штрафов.
			\item Пользователь может завершить сеанс работы в системе.
		\end{enumerate}
		
		\textbf{3.7.3 Действия библиотекаря}
		\begin{enumerate}
			\item Библиотекарь может получить доступ к функционалу оформления выдачи материала.
			\item Библиотекарь может выполнить поиск пользователя по идентификатору, имени или иному уникальному атрибуту при оформлении выдачи.
			\item Библиотекарь может выбрать материал из каталога для выдачи пользователю.
			\item Библиотекарь может указать срок возврата материала при оформлении выдачи в соответствии с внутренними правилами библиотеки.
			\item Библиотекарь может подтвердить выдачу материала пользователю.
			\item Библиотекарь может получить доступ к функционалу возврата материала.
			\item Библиотекарь может выполнить поиск записи выдачи по идентификатору пользователя, названию материала или идентификатору экземпляра при возврате.
			\item Библиотекарь может зафиксировать факт повреждения материала при его возврате, указав описание повреждения.
			\item Библиотекарь может вручную отменить начисленный штраф при малой просрочке в рамках установленных правил, подтвердив данное действие.
			\item Библиотекарь может зарегистрировать возврат материала в системе.
			\item Библиотекарь может просматривать список новых бронирований, требующих обработки.
			\item Библиотекарь может получить доступ к функционалу управления каталогом материалов для добавления, редактирования или удаления записей материалов.
		\end{enumerate}
	\end{enumerate}

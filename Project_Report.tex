...
\begin{document}
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			\item Система должна позволить неавторизованному пользователю инициировать процесс регистрации в системе.
			\item Система должна позволить неавторизованному пользователю предоставить регистрационные данные: ФИО, электронный адрес, пароль и подтверждение пароля.
			\item Система должна позволить пользователю запросить повторную отправку письма активации учётной записи.
			\item Система должна позволить пользователю, имеющему активную учётную запись, инициировать процесс входа в систему.
			\item Система должна позволить пользователю предоставить свои учётные данные (e-mail и пароль) для авторизации.
			\item Система должна позволить авторизованному пользователю завершить сеанс работы в системе.
		...
		\begin{enumerate}
			\item Система должна позволить неавторизованному пользователю просматривать каталог материалов без авторизации.
			\item Система должна позволить пользователю выполнять поиск материалов по каталогу, указав поисковый запрос.
			\item Система должна позволить пользователю применять фильтры к результатам поиска, указав параметры фильтрации (жанр, год издания, тип материала, наличие).
			\item Система должна позволить пользователю сортировать результаты поиска, выбрав критерий сортировки (по алфавиту, дате издания, дате добавления).
			\item Система должна позволить пользователю запрашивать детальную информацию о выбранном материале из результатов поиска.
		...
		\begin{enumerate}
			\item Система должна позволить авторизованному пользователю получить доступ к своему личному кабинету после авторизации.
			\item Система должна позволить авторизованному пользователю просматривать список своих активных бронирований.
			\item Система должна позволить авторизованному пользователю инициировать бронирование выбранного материала из каталога.
			\item Система должна позволить пользователю отменять созданную бронь до её подтверждения или обработки библиотекарем.
		...
		\begin{enumerate}
			\item Система должна позволить библиотекарю получить доступ к функционалу оформления выдачи материала.
			\item Система должна позволить библиотекарю выполнить поиск пользователя по идентификатору, имени или иному уникальному атрибуту при оформлении выдачи.
			\item Система должна позволить библиотекарю выбрать материал из каталога для выдачи пользователю.
			\item Система должна позволить библиотекарю указать срок возврата материала при оформлении выдачи в соответствии с внутренними правилами библиотеки.
			\item Система должна позволить библиотекарю подтвердить выдачу материала пользователю.
			\item Система должна позволить авторизованному пользователю просматривать список выданных ему материалов с информацией о дате выдачи и сроке возврата.
			\item Система должна позволить авторизованному пользователю просматривать историю выданных материалов.
		...
		\begin{enumerate}
			\item Система должна позволить библиотекарю получить доступ к функционалу возврата материала.
			\item Система должна позволить библиотекарю выполнить поиск записи выдачи по идентификатору пользователя, названию материала или идентификатору экземпляра при возврате.
			\item Система должна позволить библиотекарю зарегистрировать возврат материала в системе.
			\item Система должна позволить библиотекарю зафиксировать факт повреждения материала при его возврате, указав описание повреждения.
			\item Система должна позволить библиотекарю вручную отменить начисленный штраф при малой просрочке в рамках установленных правил, подтвердив данное действие.
		...
		\begin{enumerate}
			\item Система должна позволить авторизованному пользователю просматривать актуальный список своих активных штрафов с указанием суммы, причины начисления и срока оплаты.
			\item Система должна позволить пользователю инициировать оплату выбранного штрафа из списка активных штрафов.
		...
		\item \textbf{Управление каталогом и бронированиями (для библиотекаря)}
		\begin{enumerate}
			\item Система должна позволить библиотекарю просматривать список новых бронирований, требующих обработки.
			\item Система должна позволить библиотекарю получить доступ к функционалу управления каталогом материалов для добавления, редактирования или удаления записей материалов.
		\end{enumerate}
	\end{enumerate}
	...
	\paragraph{Клиентские приложения}
	
	Система предоставляет три типа клиентских приложений для различных платформ, обеспечивая максимальную доступность и удобство использования для разных категорий пользователей.
	
	\begin{itemize}
		\item \textbf{Library Web App} (React, Redux, TypeScript) --- веб-приложение для работы через браузер.
		
		\textit{Технологический стек:}
		\begin{itemize}
			\item \textbf{React} --- библиотека для построения пользовательских интерфейсов на основе компонентного подхода. Позволяет создавать переиспользуемые UI-компоненты, обеспечивает эффективное обновление интерфейса через виртуальный DOM.
			\item \textbf{Redux} --- библиотека управления состоянием приложения. Централизует состояние, упрощает отладку и тестирование, обеспечивает предсказуемое поведение приложения при работе с данными каталога, бронированиями и профилем пользователя.
			\item \textbf{TypeScript} --- типизированное надмножество JavaScript. Обеспечивает статическую типизацию, раннее обнаружение ошибок на этапе разработки, улучшает читаемость кода и упрощает рефакторинг больших приложений.
		\end{itemize}
		
		\textit{Особенности:}
		\begin{itemize}
			\item Используется как читателями для поиска и бронирования материалов, так и библиотекарями для управления каталогом и обработки операций выдачи/возврата.
			\item Коммуникация с backend осуществляется через REST API по протоколу HTTPS с использованием формата JSON.
			\item Для интеграции с платёжным сервисом используется механизм редиректа во внешний браузер или WebView для безопасной обработки платежей.
			\item Адаптивный дизайн обеспечивает корректную работу на различных размерах экранов (десктоп, планшет, мобильные устройства).
		\end{itemize}
		
		\item \textbf{Library Mobile App} (Flutter) --- мобильное приложение для iOS и Android.
		
		\textit{Технологический стек:}
		\begin{itemize}
			\item \textbf{Flutter} --- фреймворк от Google для разработки кроссплатформенных мобильных приложений. Использует язык программирования Dart и собственный движок рендеринга для создания нативно выглядящих интерфейсов.
		\end{itemize}
		
		\textit{Особенности:}
		\begin{itemize}
			\item Единая кодовая база для платформ iOS и Android, что значительно сокращает время разработки и поддержки.
			\item Высокая производительность благодаря компиляции в нативный код и отсутствию необходимости в мосте между JavaScript и нативными компонентами.
			\item Богатая экосистема готовых виджетов и пакетов для реализации типичных мобильных паттернов (навигация, формы, списки, уведомления).
			\item Поддержка push-уведомлений для информирования пользователей о статусе бронирований, напоминаниях о сроках возврата и других важных событиях.
			\item Коммуникация с backend через HTTPS/REST API в формате JSON, аналогично веб-приложению.
		\end{itemize}
		
		\item \textbf{Library Desktop App} (Compose Multiplatform Desktop, Kotlin) --- нативное десктопное приложение.
		
		\textit{Технологический стек:}
		\begin{itemize}
			\item \textbf{Compose Multiplatform Desktop} --- современный фреймворк для создания десктопных приложений на основе декларативного подхода к построению UI, аналогичного Jetpack Compose для Android.
			\item \textbf{Kotlin} --- статически типизированный язык программирования, работающий на JVM. Обеспечивает безопасность типов, лаконичный синтаксис, отличную интеграцию с Java-библиотеками.
		\end{itemize}
		
		\textit{Особенности:}
		\begin{itemize}
			\item Нативный пользовательский интерфейс, соответствующий стандартам операционных систем (Windows, macOS, Linux).
			\item Высокая производительность за счёт компиляции в нативный код и оптимизации для десктопных платформ.
			\item Возможность работы в офлайн-режиме с синхронизацией данных при восстановлении соединения.
			\item Поддержка клавиатурных сокращений и других десктопных паттернов взаимодействия для повышения эффективности работы библиотекарей.
			\item Прямая интеграция с системными уведомлениями операционной системы.
			\item Коммуникация с backend через HTTPS/REST API в формате JSON.
		\end{itemize}
	\end{itemize}
	
	Все три клиентских приложения взаимодействуют с единым backend-сервисом \textbf{Library API} (Java + Spring REST), что обеспечивает консистентность данных и бизнес-логики независимо от платформы клиента. Каждое приложение оптимизировано под свою целевую платформу, но предоставляет пользователям единообразный функционал системы управления библиотекой.
